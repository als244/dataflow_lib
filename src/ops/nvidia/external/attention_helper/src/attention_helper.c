...
int flash3_attention_fwd(Dataflow_Handle * dataflow_handle, int stream_id, Op * op, void * op_extra) {
	...
	int num_seqs = *((int *)(op_args[0]));
	int total_q = *((int *)(op_args[1]));
	int total_k = *((int *)(op_args[2]));
	int * cum_q_seqlens = *((int **) op_args[3]);
	int max_seqlen_q = *((int *)(op_args[4]));
	int * k_seqlens = *((int **) op_args[5]);
	int max_seqlen_k = *((int *)(op_args[6]));
	int flash_dtype_as_int = *((int *)(op_args[7]));
