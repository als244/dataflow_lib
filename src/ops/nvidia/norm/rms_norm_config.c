...
int rms_norm_bwd_x_set_launch_config(Cuda_Launch_Config * cuda_launch_config, Dataflow_Handle * dataflow_handle, Cuda_Function * cuda_function, Op * op) {
	...
	// backward dtype
	DataflowDatatype sq_sums_dt = arg_dtypes[3];

	size_t sq_sums_dtype_size = dataflow_sizeof_element(sq_sums_dt);

	if (sq_sums_dtype_size == 0){
		fprintf(stderr, "Error: rms norm not available for dtype %s...\n", dataflow_datatype_as_string(sq_sums_dt));
		return -1;
	}
	...
	int rms_base_smem = sq_sums_dtype_size * model_dim;
