...
int main(int argc, char * argv[]){
	...
	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_attn_norm, attn_norm, attn_norm_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for attn norm weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_wq, wq, wq_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for attn query proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_wk, wk, wk_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for attn key proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_wv, wv, wv_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for attn value proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_wo, wo, wo_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for attn output proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_ffn_norm, ffn_norm, ffn_norm_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for ffn norm weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_w1, w1, w1_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for ffn w1 proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_w2, w2, w2_size);
	if (ret){
		fprintf(stderr, "Error: host to device transfer failed for ffn w2 proj weights...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_w3, w3, w3_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_inbound_transfer(&cuda_dataflow_handle, inbound_stream_id_a, d_seq_positions, seq_positions, seq_positions_size);
	if (ret){
	...
	ret = submit_rope(&cuda_dataflow_handle, compute_stream_id_a, 
						fwd_dt, 
	...
	ret = submit_swiglu(&cuda_dataflow_handle, compute_stream_id_a, 
						fwd_dt, 
						total_q, (int) ffn_dim, 
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, attn_norm_weighted_sums, d_attn_norm_weighted_sums, weighted_sums_size);
	if (ret){
		fprintf(stderr, "Error: could not submit outbound transfer for attn_norm_weighted_sums...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, attn_norm_rms_vals, d_attn_norm_rms_vals, rms_vals_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, wq_out, d_wq_out, x_size);
	if (ret){
		fprintf(stderr, "Error: could not submit outbound transfer for wq_out (post rope)...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, wk_out, d_wk_out, x_kv_size);
	if (ret){
		fprintf(stderr, "Error: could not submit outbound transfer for wk_out (post rope)...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, wv_out, d_wv_out, x_kv_size);
	if (ret){
	...
	// Attention
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, attn_out, d_attn_out, x_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, attn_softmax_lse, d_attn_softmax_lse, softmax_lse_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, wo_out, d_wo_out, x_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, ffn_norm_weighted_sums, d_ffn_norm_weighted_sums, weighted_sums_size);
	if (ret){
		fprintf(stderr, "Error: could not submit outbound transfer for ffn_norm_weighted_sums...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, ffn_norm_rms_vals, d_ffn_norm_rms_vals, rms_vals_size);
	if (ret){
	...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, w1_out, d_w1_out, x_ffn_size);
	if (ret){
		fprintf(stderr, "Error: could not submit outbound transfer for w1_out...\n");
		return -1;
	}

	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, w3_out, d_w3_out, x_ffn_size);
	if (ret){
	...
	// SwiGLU Activation...
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, swiglu_out, d_swiglu_out, x_ffn_size);
	if (ret){
	...
	// W2 Projection
	// (Layer output...)
	ret = cuda_dataflow_handle.submit_outbound_transfer(&cuda_dataflow_handle, outbound_stream_id_a, w2_out, d_w2_out, x_size);
	if (ret){
