...
// otherwise 0
int init_table(Table * table, Hash_Func hash_func, uint64_t key_size_bytes, uint64_t value_size_bytes, 
...
uint64_t get_next_ind_table(uint64_t * is_empty_bit_vector, uint64_t table_size, uint64_t start_ind){
...
int insert_table(Table * table, void * key, void * value) {
	...
	uint64_t ret = find_table(table, key, false, NULL);
	if (ret != -1){
	...
	uint64_t insert_ind = get_next_ind_table(table -> is_empty_bit_vector, table -> size, hash_ind);
...
long find_table(Table * table, void * key, bool to_copy_value, void ** ret_value){
	...
	uint64_t next_empty = get_next_ind_table(is_empty_bit_vector, size, hash_ind);
...
int remove_table(Table * table, void * key, void * ret_value) {
	...
	long empty_ind = find_table(table, key, true, (void **) ret_value);
	...
	uint64_t value_size_bytes = (table -> config).value_size_bytes;
	uint64_t * is_empty_bit_vector = table -> is_empty_bit_vector;
	uint64_t next_empty = get_next_ind_table(is_empty_bit_vector, size, empty_ind);
